...
def get_batches():
    ...
    try:
        ...
        cur.execute(
            "SELECT id, name FROM batches WHERE dojo_id=%s ORDER BY name",
            (dojo_id,)
        )
...
def mark_attendance():
    ...
    for r in records:
        if r.get('status') not in valid_statuses:
            return jsonify({
                "error": f"Invalid status: {r.get('status')}"
            }), 400
    ...
    try:
        ...
        for r in records:
            ...
            cur.execute("""
                ...
                (student_id, status, date, class_number)
                VALUES (%s, %s, %s, 1)
...
def get_report():
    ...
    if not (dojo_id and batch_id and att_date):
        return jsonify({
            "error": "dojo_id, batch_id, date required"
        }), 400
